...
class AGCInstanceImpl implements AGCInstance {
  ...
  init(applicationContext: common.Context, config?: any): AGCInstance {
    ...
    if (applicationContext?.getApplicationContext()) {
      this._appContext = applicationContext?.getApplicationContext();
    }
    AegisAesManager.init(this._appContext);
    this._servicesConfig = new ServicesConfigImpl(this._appContext, config);
    return this;
  }
